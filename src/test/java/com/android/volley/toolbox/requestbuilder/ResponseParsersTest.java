...
public class ResponseParsersTest {
    ...
    public void correctParsingForString() throws Exception {
        ...
        String result =
                ResponseParsers.forString().parseNetworkResponse(new NetworkResponse(data)).result;
    ...
    public void correctParsingForJSONObject() throws Exception {
        String jsonString =
                new JSONObject().put("first-key", "first-value").put("second key", 3).toString();
        byte[] data = stringBytes(jsonString);

        JSONObject result =
                ResponseParsers.forJSONObject()
    ...
    public void correctParsingForJSONArray() throws Exception {
        String jsonString = new JSONArray().put("first-key").put(3).toString();
        byte[] data = stringBytes(jsonString);

        JSONArray result =
                ResponseParsers.forJSONArray()
                        .parseNetworkResponse(new NetworkResponse(data))
                        .result;
